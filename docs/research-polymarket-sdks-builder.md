...
De `wallets-auth`: *"attaching a builder code does NOT require the builder's secret — only the `bytes32` builder field value. The builder's secret remains server-side and never touches the client or order submission."*
