...
<a id="ac4-7"></a>
**AC4.7** (Complex, in-widget detail view + focus handoff) — **RECONCILED per Decision D11** (see [`docs/widget-form-factor.md`](../../docs/widget-form-factor.md); supersedes the prior dialog/sheet presentation of the detail).
WHILE the market detail is open, THE SYSTEM SHALL present it as an **in-widget view** within the compact widget's internal view-stack — a labelled `region` (`aria-label`/`aria-labelledby` referencing the question), **not** a `role="dialog"` — carrying a visible **Back** affordance that returns to the browse view (the shell restoring scroll and returning focus to the invoking control), and SHALL move focus to the detail heading on open (a handoff, never a focus trap). The detail body scrolls within the widget's bounded, `overflow-y:auto` body. Dialog semantics (`role="dialog"` + `aria-modal="true"` + `aria-labelledby` + a defined initial focus target + Escape + backdrop dismissal + focus return) now apply ONLY to the **Settings** modal, which is scoped over the widget (SJModal). *Superseded prior wording: the detail opened as a `role="dialog"` full-screen/bottom sheet on mobile becoming a centered dialog at `md`+.*
...
- **US4 — Detail.** Given a market card, When selected, Then the detail opens as an **in-widget view** (a labelled region with a Back affordance, per D11 — not a dialog) with outcomes signalled by triad+text (neutral chips when >2 outcomes), focus moves to the detail heading on open, selection drives the bet form, and Back returns to browse restoring focus (AC4.1–AC4.7).
...
<a id="nfr-mf-5"></a>
**NFR-MF-5** (Ubiquitous, compact-widget layout + responsive host) — **RECONCILED per Decision D11** (see [`docs/widget-form-factor.md`](../../docs/widget-form-factor.md); the two-pane matrix below is SUPERSEDED for the widget itself).
THE SYSTEM SHALL deliver the product as a **compact embeddable widget** — a bounded card (`max-inline-size ≈ 28rem`, fluid to `width:100%` below that) that uses a **single-column internal view-stack at ALL sizes** (never a two-pane list+detail, never a multi-column card grid):
- **The widget (all breakpoints):** single-column stack; a compact scrollable market list inside the widget's bounded, `overflow-y:auto` body; the market detail as an **in-widget view with Back** (AC4.7); a Positions view reachable from the widget nav; Settings as the only modal. Mobile-first still holds (base = mobile; the widget is already ~mobile-width).
- **The host page** is what is responsive around the widget: it SHALL provide a concrete responsive layout at the DS breakpoints (e.g. a stacked column on mobile, a content column + widget aside at `md`+), and SHALL cap text blocks with a `max-inline-size` so host content never runs full-bleed at `xl`/`2xl`.
- *Superseded prior matrix (predicated on a full-page layout): base single-column with the detail as a mobile modal/sheet; a two-pane list+detail + multi-column card grid + positions table at md/lg; capped text at xl/2xl.* Positions still render as stacked, wrapping rows (no horizontal scroll), promoting to a table at the wider breakpoints WHERE the widget's positions view has the room.
...
Per Decision D11 the composition is a **compact embeddable widget** (`PolymarketWidget`) embedded in a demo host page (`App.vue`), with an internal view-stack: Widget header (title + Settings entry, US8) → `WMarketSearch` (US2) → compact single-column `WMarketList`/`WMarketCard` (US3) → in-widget `WMarketDetail` view with Back (US4) → `WBetForm` + `WBetReceipt` (US5) → `WPositions` view (US6) → `WAiPrediction` opt-in inside detail (US7). Settings is the only modal (SJModal), scoped over the widget. See [`docs/widget-form-factor.md`](../../docs/widget-form-factor.md).
