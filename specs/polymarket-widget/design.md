...
> Anchored to `spec.md` (US1–US10, AC1.*–AC10.*, NFR-*, decisions D1–D12). Read that file first — this document does not restate rationale, only the concrete shape of the code that satisfies it.
...
export function getMarkets(filters?: MarketListFilters, opts?: { signal?: AbortSignal }): Promise<Market[]>;
...
|---|---|---|---|---|
| Search (`WMarketSearch`/`WMarketList`) | `SJSkeleton` list | "No markets matched" (explicit, AC2.4) | error text + Retry button, previous results *not* discarded silently (AC2.5) | `role="status" aria-live="polite"` while loading; `role="alert"` on error |
...
| AC5.1–AC5.9 | `services/betting.service.ts` (`MockBettingService`, `computeFees`), `models/bet.ts`, `stores/bets.store.ts`, `components/widget/WBetForm.vue`, `WBetReceipt.vue`, `config/builder.config.ts` |
